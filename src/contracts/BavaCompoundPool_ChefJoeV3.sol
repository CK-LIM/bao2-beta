...
contract BavaCompoundPool_ChefJoeV3 is BRTERC20 {
    ...
    struct PoolRestakingInfo {
        IJoeChefV3 stakingContract;   // TraderJoe LP Staking contract
    ...
    function initPool(IERC20 _lpToken, IJoeChefV3 _stakingPglContract, uint256 _restakingFarmID, IERC20 _rewardToken, IERC20[] memory _bonusRewardTokens, IRouter _router, uint256 _MIN_TOKENS_TO_REINVEST, uint256 _DEV_FEE_BIPS, uint256 _REINVEST_REWARD_BIPS) external onlyOwner {        
        ...
        poolRestakingInfo.stakingContract = _stakingPglContract;
    ...
    function setAllowancesStaking(uint256 _amount) external onlyOwner {
        ...
        if (address(poolRestaking.stakingContract) != address(0)) {
            poolInfo.lpToken.approve(address(poolRestaking.stakingContract), _amount);
        }
    }
  ...
  // Withdraw LP tokens from BavaMasterFarmer. argument "_amount" is receipt amount.
    ...
    function _stakeDepositTokens(uint amount) private {
        ...
        poolRestakingInfo.stakingContract.deposit(poolRestakingInfo.restakingFarmID, amount);
    }

    // Withdraw LP token to 3rd party restaking farm
    function _withdrawDepositTokens(uint amount) private {
        require(amount > 0, "#<0");
        (uint256 depositAmount,) = poolRestakingInfo.stakingContract.userInfo(poolRestakingInfo.restakingFarmID, address(this));
    ...
    function _getReinvestReward() private {
        poolRestakingInfo.stakingContract.withdraw(poolRestakingInfo.restakingFarmID, 0);
    }
    ...
    function emergencyWithdrawDepositTokens(bool disableDeposits) external onlyOwner {
        ...
        poolRestaking.stakingContract.emergencyWithdraw(poolRestaking.restakingFarmID);
    ...
    function checkReward() public view returns (uint, uint[] memory) {
        PoolRestakingInfo storage poolRestaking = poolRestakingInfo;
        (uint256 pending, address bonusTokenAdd, , uint256 pendingBonusTokenAmount) = poolRestaking.stakingContract.pendingTokens(poolRestaking.restakingFarmID, address(this));
        uint256 rewardBalance = rewardToken.balanceOf(address(this));

        uint256 bonusRewardBal;
        uint256[] memory pendingBonusToken;

        if(bonusRewardTokens.length > 0) {
            if (bonusTokenAdd != address(0)) {
                for (uint i; i < bonusRewardTokens.length; i++) {
                    bonusRewardBal = bonusRewardTokens[i].balanceOf(address(this));
                    pendingBonusToken[i] = bonusRewardBal + pendingBonusTokenAmount;
                }
            }
        }
        return (pending+rewardBalance, pendingBonusToken);
    }
    ...
    function setPoolRestakingInfo(IJoeChefV3 _stakingContract, uint256 _restakingFarmID, IERC20 _rewardToken, IERC20[] memory _bonusRewardTokens, bool _withUpdate) external onlyOwner {
        require(address(_stakingContract) != address(0) , "0Addr");        
        if (_withUpdate) {
            updatePool();
        }
        poolRestakingInfo.stakingContract = _stakingContract;
        poolRestakingInfo.restakingFarmID = _restakingFarmID;
        rewardToken = _rewardToken;
        bonusRewardTokens = _bonusRewardTokens;
    }
    ...
    ****************************************/
    function _reinvest() private {
        ...
        poolRestaking.stakingContract.deposit(poolRestaking.restakingFarmID, liquidity);
